...
def main():
    ...
    def Exp():
        ...
        labelnameexp.grid(row=1, column=1, columnspan=2)
        ...
        # btnExpAnData = Button(exp_frame, text="Экспорт аналитических \n данных по работе", bg='thistle1')
        # btnExpAnData.grid(row=10, column=1)
        ...
        def ExpMin(but_id):
            ...
            allvals = []
            zeroline = (0, "0")
            allvals.append(zeroline)
            for index, r in enumerate(cursor.fetchall()):
                allvals.append(r)
                # print(index, r)
                # print(r[1])
            # print("allvals", *allvals, sep="\n")
            all1 = []
            for r in allvals:
                all1.append(r[1])
                # print(r[1])
            ...
            allind = []
            for i in range(len(all1)-1):
                if all1[i+1] == all1[i]:
                    print("равны", i, all1[i+1], all1[i])
                    allind.append(1)
                else:
                    print("НЕ равны", i, all1[i+1], all1[i])
                    allind.append(0)
            print(allind)
            ...
            print(all1)
            allvals.pop(0)
            allvalslist = [list(ele) for ele in allvals]
            for i, r in enumerate(allvalslist):
                r[1] = all1[i]
            print("allvals", *allvalslist, sep="\n")

            for r in allvalslist:
            ...
            label1_ST = Label(table1_frame, text="Уточнение", bg='thistle1')
            # global rced
            # global rced_od
            # global rced_st
            def getST1(event):
            ...
            allvals2 = []
            zeroline2 = (0, "0")
            allvals2.append(zeroline2)
            for index, r in enumerate(cursor.fetchall()):
                allvals2.append(r)
                # print(index, r)
                # print(r[1])
            # print("allvals2", *allvals2, sep="\n")
            all12 = []
            for r in allvals2:
                all12.append(r[1])
                # print(r[1])
            ...
            allind2 = []
            for i in range(len(all12) - 1):
                if all12[i + 1] == all12[i]:
                    print("равны", i, all12[i + 1], all12[i])
                    allind2.append(1)
                else:
                    print("НЕ равны", i, all12[i + 1], all12[i])
                    allind2.append(0)
            print(allind2)

            all12.pop(0)
            for i in range(len(all12)):
                if allind2[i] == 1:
                    all12[i] = ""
            print(all12)
            allvals2.pop(0)
            allvalslist2 = [list(ele) for ele in allvals2]
            for i, r in enumerate(allvalslist2):
                r[1] = all12[i]
            print("allvals2", *allvalslist2, sep="\n")

            for r in allvalslist2:
            ...
            meth3_textbox = Text(table3_frame, height=10, width=70)
            ...
            btnChangeMeth = Button(table3_frame, text="Изменить методику", command=change_meth, bg='thistle1')
            btnChangeMeth.grid(row=17, column=1)

            # btnExpExp = Button(table3_frame, text="Создать отчет \n по эксперименту", bg='thistle1')
            # btnExpExp.grid(row=18, column=1)
            ...
            btnChangeName = Button(table3_frame, text="Изменить", command=change_name, bg='thistle1')
            btnChangeName.grid(row=0, column=2)
    ...
    if len(respr) >1:
        ...
        listofres = Listbox(up_frame, listvariable=res_var, selectmode='single', width=60, height=4)
    ...
    def ReagSearch():
        ...
        #поиск по прекурсору
        labelreag1 = Label(rq_frame, text="Выберите исходное вещество", bg='thistle1', width = 30)
        ...
        #поиск по активному
        labelreag2 = Label(rq_frame, text="Выберите активное вещество", bg='thistle1', width = 30)
        ...
        # label_user = Label(rq_frame, text="Выберите пользователя", bg='thistle1', width = 30)
        # label_user.grid(row=3, column=0)

        # cbx_user = ttk.Combobox(rq_frame, values=[], width=35)
        # cbx_user.grid(row=3, column=1)
        # cbx_user['values'] = ["Демкин К.", "Комарова Д."]
        ...
        def rg4():
            ...
            if rg1 is None:
                ...
                text1 = "ADValue is not null and idReagent = " + str(rg1) + " "
                print(text1)
            ...
            if rg2 is None:
                ...
                text2 = "ADValue is not null and idReagent = " + str(rg2) + " "
                print(text2)
                thelist.append(text2)
            print("here", thelist)
            if not thelist:
                print("empty")
                thelist.append(" ADValue is not null and idReagType = 1 ")
            chibi_out = []
            request = ""
            # reques = []
            ct = int()
            ...
            print("ct", ct)
            if chubby_out:
                cursor.execute("select "+ sdl +" from ( select idExperiment, Pers_Name, RPName, ExpName, "
                                "idExperimentDate, ReagName1, sample, idAnalyticalData, AnDataName, ADValue, ADUnitsSN, "
                                               "count(*)  over (partition by ExpName, sample) as ct "
                                               "from experiment_has_analyticaldata "
                               ...
                               "join reagtype on idReagType = ReagType_idReagtype where " + chubby_out +" "
                                "and Secrecy = 0) table1 "+ct_text+" order by idExperiment, idAnalyticalData")
                ...
                btnBack.grid(row=2, column=0, columnspan=2)
                for r in cursor.fetchall():
                ...
                # btngetExpSer = Button(rq_frame, text="Создать отчет", bg='thistle1')
                # btngetExpSer.grid(row=5, column=0, columnspan=2)

                def showmeth(event):
                    row_vals = []
                    for i in tablergs.selection():
                        rowss = tablergs.item(i)
                        row_vals = rowss['values']
                    methname = row_vals[2]
                    pers_name =  row_vals[0]
                    work_name = "\n" + row_vals[1]
                    print(methname)
                    methwindow = Tk()
                    w =580
                    ...
                    x = 1000
                    y = 600
                    methwindow.geometry('%dx%d+%d+%d' % (w, h, x, y))
                    methwindow.configure(bg = "light blue")
                    methwindow.title('Методика эксперимента {}'.format(methname))
                    methbox = Text(methwindow, height=10, width=70)
                    methbox.grid(row=0, column=0)
                    cursor.execute("select MetName, Meth_text, idMethodology from methodology "
                                   "join experiment on Experiment_idExperiment = idExperiment "
                                   "where ExpName = '" + methname + "' ")
                    meth_reags = cursor.fetchone()
                    # meth_reags = meth3_rec[0]
                    meth_reags_txt = "\n" + meth_reags[1]
                    # idmeth = meth3_rec[2]
                    methbox.insert("1.0", pers_name)
                    methbox.insert("2.0", work_name)
                    methbox.insert("3.0", meth_reags_txt)

                tablergs.bind("<<TreeviewSelect>>", showmeth)


        btngetreag4 = Button(rq_frame, text="Выбрать", command=rg4, bg='thistle1')
        btngetreag4.grid(row=4, column=0, columnspan=2)
        ...
        btnBack = Button(rs_frame, text="Вернуться на начальную страницу.", command=Back, bg='thistle1')
        btnBack.grid(row=2, column=0, columnspan=2)
    ...
    def Reag():
        ...
        labelstore = Label(rogs_frame, text='Запасы реагентов', width=30, bg='thistle1')
        labelstore.grid(row=0, column=0, columnspan=4)
        ...
        def chose_reag():
            ...
            table5_frame.grid(row=7, column=0)
            ...
            def hide():
                ...
                y = (hs / 2) - (h / 2)
                root.geometry('%dx%d+%d+%d' % (w, h, x, y))
            btnHide = Button(rogs_frame, text="Скрыть", command=hide, bg='thistle1', width=30)
            btnHide.grid(row=9, column=0)
            # btnExpStore = Button(rogs_frame, text="Создать отчет", bg='thistle1', width=30)
            # btnExpStore.grid(row=8, column=0)

        btnBack = Button(rogs_frame, text="Показать использования", command=chose_reag, bg='thistle1', width=30)
        btnBack.grid(row=3, column=0)

        btnExpStore = Button(rogs_frame, text="Создать отчет", bg='thistle1', width=30)
        btnExpStore.grid(row=4, column=0)
        ...
        btnBack = Button(rogs_frame, text="Вернуться на начальную страницу", command=Back, bg='thistle1', width=30)
