...
def main():
    ...
    def Exp():
        ...
        exp_out_frame = Frame(root, bg='light blue')
        exp_out_frame.grid(row=0, column=0, padx = 10, pady=10)

        # canv = Canvas(exp_out_frame, bg='light blue')
        # canv.grid(row=2, column=0)
        #
        # exp_frame = Frame(canv, bg='light blue')

        exp_frame = Frame(exp_out_frame, bg='light blue')
        exp_frame.grid(row=2, column=0, padx = 10, pady=10)

        labelmyexp = Label(exp_out_frame, text = "Мои эксперименты", bg = 'thistle1')
        labelmyexp.grid(row=0, column=0)
        labelnameexp = Label(exp_out_frame, text="Научно-исследовательская работа \n '{}'".format(papername), bg='thistle1')
        labelnameexp.grid(row=1, column=0)
        ...
        # listofexp = Listbox(exp_out_frame, listvariable=exp_var)
        # listofexp.grid(row=0, column=5)
        # expscroll = Scrollbar(exp_out_frame, command=listofexp.yview)
        # expscroll.grid(row=0, column=6, sticky='ns')
        ...
        def ExpMin(but_id):
            ...
            labelexpname = Label(emout_frame, text="{}".format(butname[but_id]), bg='thistle1')
            ...
            cursor.execute("select " + headosreag + " from reagusage join reagent on idReagent = Reagent_idReagent "
                           ...
                           "join units on idUnits = Units_idUnits "
                            ...
                            "where Experiment_idExperiment = " + str(btid) + " and "
            ...
            # labtab1 = Label(table1_frame, text='Использование реагентов', bg='thistle1')
            ...
            def T1Add():
                global tb12
                if cbxST1.get() and cbxRG.get() and entryAMRG.get():
                    ...
                    cursor.execute("insert into reagusage (idReagUsage, AmUsed, Reagent_idReagent, Process_idProcess, "
            ...
            btnT1AddCom.grid(row=7, column=4)
            ...
            cursor.execute("select " + headoseq + " from equipusage "
                           ...
                           "join experimentdate on idExperimentDate = ExperimentDate_idExperimentDate "
                            "join stagetype on idStageType = StageType_idStageType "
                            "join processstage on idProcessStage = ProcessStage_idProcessStage "
                           "where Experiment_idExperiment = " + str(btid) + " and ExperimentDate_idExperimentDate <> 1;")
            ...
            btnT2AddCom.grid(row=7, column=4)
            ...
            # label3 = Label(table3_frame, text = 'Методика', bg='thistle1')
            ...
            cursor.execute("select sample from experiment_has_analyticaldata where Experiment_idExperiment= " +btid + " "
                            "group by Experiment_idExperiment, sample")
            sample = []
            for row in cursor.fetchall():
                sample.append(row[0])
                ...
                for num in range(len(new_val_many)):
                    if prev_val_many[num] != new_val_many[num]:
                        ...
                        if not new_val_many[num]:
                            ...
                            print(values4)
                            cursor.execute("insert into experiment_has_analyticaldata values(%s, %s, %s, %s, %s) ", values4)
                            cnx.commit()
        ...
        def Back():
            ...
            parent1.grid_remove()
            main()
        btnBack = Button(exp_out_frame, text="Вернуться на начальную страницу.", command=Back, bg='thistle1')
        btnBack.grid(row=k+2, column=0)
            ...
            if chubby_out:
                cursor.execute("select "+ sdl +" from experiment_has_analyticaldata "
                               ...
                               "join reagtype on idReagType = ReagType_idReagtype where " + chubby_out +" "
                                "and Secrecy = 0 order by ExpName")
